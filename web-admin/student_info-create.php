...
$student_id = $student_name = $course_name = $term = $cgpa = "";
$student_id_err = $student_name_err = $course_name_err = $term_err = $cgpa_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($student_id_err) && empty($student_name_err) && empty($course_name_err) && empty($term_err) && empty($cgpa_err)){
        // Prepare an insert statement
        $sql = "INSERT INTO student_info (student_id, student_name, course_name, term, cgpa) VALUES (?, ?, ?, ?, ?)";

        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssss", $param_student_id, $param_student_name, $param_course_name, $param_term, $param_cgpa);
